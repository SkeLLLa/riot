...
var baf = (function( maxBatchSize, minBatchSize ) {
  'use strict';
  var batching, reqFrames = [], calls = {
    raf: 0,
    batch: 0
  }

  function add( callback ) {
    reqFrames.push( callback );
    calls.batch++
    if ( batching || minBatchSize > reqFrames.length ) return true;
    batching = requestAnimationFrame( function() {
      batching = false;
      calls.raf++
      var batchLength = typeof maxBatchSize === 'undefined' ? reqFrames.length : maxBatchSize;

      var animationFrames = reqFrames.splice( 0, batchLength ),
        i;
  ...
  function cancel( callback ) {
    for ( var i = 0; i < reqFrames.length; i++ ) {
      if ( reqFrames[ i ] === callback ) {
        reqFrames.splice( i--, 1 );
      }
    }
  }
...
setTimeout(function(){
  'use strict';
  console.log('Renderer stats', baf.stats);
}, 10000)
  ...
  parent.one( 'before-mount', function() {
    ...
    function renderItem() {
      ...
      if (
        ...
        if ( isVirtual ) tag._root = tag.root.firstChild // save reference for further moves or
                                                         // inserts
        ...
        // this tag must be insert
          ...
          else root.insertBefore( tag.root, tags[ i ].root ) // #1374 some browsers reset
                                                             // selected here
      ...
      // but we need to track the `myTag.tags['child']` property looping a custom child node
      // named `child`
      if ( child ) parent.tags[ tagName ] = tags
      return baf.add(renderItem)
    }

    //while ( il < itemsLength ) {
    //  il++;
    //  requestAnimationFrame( renderItem )
    //}
    //}
    //FIXME: hack for start batching
